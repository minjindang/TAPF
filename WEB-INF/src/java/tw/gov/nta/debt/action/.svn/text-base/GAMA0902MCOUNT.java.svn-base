...
public class GAMA0902MCOUNT extends DefaultAction {
	...
	private BigDecimal getOriginInterestAmount(Cima0101Form myform, List list) {
		...
		//取得發行及結束日期的年度
		int issueYear = calendar1.get(Calendar.YEAR),dueYear = calendar2.get(Calendar.YEAR);
		...
		if (!calendar1.getTime().equals(calendar2.getTime())) {
			originInterestAmount = originInterestAmount.add(interestCounter.countInterest(calendar1.getTime(),calendar2.getTime(),list,
