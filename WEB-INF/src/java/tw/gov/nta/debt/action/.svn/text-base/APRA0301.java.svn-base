...
import tw.gov.nta.debt.form.Apra0301Form;

import com.kangdainfo.ast.sql.ConnectionSQLRunner;
import com.kangdainfo.ast.sql.SQLJob;
import com.kangdainfo.ast.sql.SQLRunner;

public class APRA0301 extends DefaultAction {

	protected SQLJob delTemp(ActionForm form,String usrId){
		SQLJob sqljob = new SQLJob();
		sqljob.appendSQL("DELETE FROM APRA03_RPT ");
	...
	protected SQLJob insTemp(ActionForm form,String usrId){
		Apra0301Form apra0301form = (Apra0301Form) form;
		SQLJob sqljob = new SQLJob();
		sqljob.appendSQL("INSERT INTO APRA03_RPT ");
		sqljob.appendSQL(" ( ");
		...
		sqljob.appendSQL(" WHERE I.ID=D.ISSUE_ID AND D.ID=P.DEBT_ID AND I.DEBT_TYPE='A' AND D.BUDGET_CODE='2'"); //BUDGET_CODE 特別預算代碼2 要隨資料庫更動
		if( null != apra0301form.getRepaySdate())
		{
			sqljob.appendSQL("AND P.REPAY_DATE>=? ");
			sqljob.addParameter(apra0301form.getRepaySdate());
		}

		if( null != apra0301form.getRepayEdate())
		{
			sqljob.appendSQL("AND P.REPAY_DATE<=? ");
			sqljob.addParameter(apra0301form.getRepayEdate());
		}
