...
* Last Modified..: $Date: 2006/06/27 09:24:52 $
...
*/
	function dynCalendar(objName, callbackFunc)
	{
        ...
        */
		...
		if(objName != "myCalendar")
			this.writeHTML();
	}
	
	dynCalendar.prototype.callBackObj;
	dynCalendar.prototype.setCallBackObj = function(obj){
	...
	function dynCalendar_show()
	{
		...
		while(i <= numdays){
            ...
            */
			...
			if(this.objName	 == "myCalendar"){//新用法
        ...
        */
		if (this.useYearCombo) 
		{
			...
			for (i = thisMonth.getFullYear() - this.yearComboRange; i <= (thisMonth.getFullYear() + this.yearComboRange); i++) 
			{
				...
				yearComboOptions += sprintf('<option value="%s" %s>%s</option>', i, selected, i-1911); //sam 20041028
			}
...
*/
	function dynCalendar_writeHTML()
	{
		if (is_ie5up || is_nav6up || is_gecko) {
			document.write(sprintf('<a href="javascript: %s.show()"><img src="%sdynCalendar.gif" border="0" align="absmiddle" width="16" height="16" title="選擇日期"/></a>', this.objName, this.imagesPath));
...
*/
	function dynCalendar_getLayer()
	{
		...
		if (document.getElementById(layerID)) {
			...
			var tNewObj = document.createElement("DIV");
			tNewObj.className = "dynCalendar";
			tNewObj.id = "dynCalendar";
			tNewObj.onmouseover="myCalendar._mouseover(true)";
			tNewObj.onmouseout="myCalendar._mouseover(false)";
			var tForm = document.getElementsByTagName("form");
			if(tForm && tForm.length > 0)
				document.getElementsByTagName("form")[0].appendChild(tNewObj);
			return tNewObj;
		}
...
function dynCalendar_Callback(obj, date, month, year)
{
	if(year-1911 < 100) {year = '0' + (year-1911)} else {year = ''+(year-1911)} ;
	if(month < 10) month = '0' + month ;
	if(date < 10) date = '0' + date ;
	var myObject = document.getElementsByName(obj)[0];
	myObject.value = year + month + date;
	if(myObject.onblur)
		myObject.onblur();
}
var myCalendar = new dynCalendar('myCalendar', 'dynCalendar_Callback', '/tapf/assets/images/calendar/','dynCalendar');