...
//----------------------------------------------------------------------------------------
//檢查form上的欄位資料是否為null或空字串，若有則回應所傳入參數Cols的順序號碼，若無則傳回-1
//var form:要檢查的form物件
//var cols:欄位名稱陣列
//作者：李偉銘
//--------------------------------------------------------------------------------------------
function checkFormEmpty(form, cols){
	if(form == null || cols == null || cols.length == 0)
		return -1;
	for(var i = 0 ; i < cols.length ; i++){
		if(form.elements[cols[i]] != null && form.elements[cols[i]].value != null && form.elements[cols[i]].value.replace(/(^\s*)|(\s*$)/g, "") == ""){
			form.elements[cols[i]].focus();
			return i;
		}
	}
	return -1;
