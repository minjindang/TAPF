	...
	<tr>
		...
		<td class='table_td'>
			<select name="issueKind">
				<option value="0" <c:if test="${AccountQueryForm.issueKind eq '0'}">selected</c:if>>登錄</option>
				<option value="1" <c:if test="${AccountQueryForm.issueKind eq '1'}">selected</c:if>>債票</option>
			</select>
		</td>
	</tr>
</table>