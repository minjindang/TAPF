...
function countTotal(form)
{
	...
	var allMoney = srcMoney-founda-foundb-lastFound;
	var nowFound = srcMoney-founda-foundb-lastFound;
	if(allMoney<0)
	{
...
function countFound(form)
{
	...
	if(null == lastFound || "" == lastFound)
	{
		...
		if(lastFound > 0){
			...
			form.elements['bigDecimal(allMoney)'].value = srcMoney;//本次撥付金額
		}else{
			form.elements['bigDecimal(nowFound)'].value = srcMoney;//本年度受分配金額
