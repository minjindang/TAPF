...
function getAuditAmount(form)
{
	...
	//document.getElementById("oldKeepAmount").innerHTML = addMoneyMask(keepAmount);
}
function getOldKeepAmount(form)
{
	var keepAmount = parseFloat(replaceAll(form.elements['bigDecimal(keepAmount)'].value,",",""));
	document.getElementById("oldKeepAmount").innerHTML = addMoneyMask(keepAmount);
	form.elements['bigDecimal(oldKeepAmount)'].value = addMoneyMask(keepAmount);
}
function getKeepAmount(form)
{
	...
	var plusdebtAmount = parseFloat(replaceAll(form.elements['bigDecimal(plusdebtAmount)'].value,",",""));
...
{
	var mainAccountYear = "<%=request.getAttribute("mainAccountYear")%>";
	if(form.dataType.value!="" && form.banish.value !="")
		doAction("GAMA0502A");
}
