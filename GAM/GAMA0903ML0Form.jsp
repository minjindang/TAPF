	...
	<tr>
		<td>
			<display:table class="table_list" name="ratelist" id="row">
				...
				%>  
					<html:link action='/GAMA0904' name='paramMap'>
						<logic:notEmpty name='row' property='effectiveDate'>
							<bean:define id="effectiveDate" name='row' property='effectiveDate' type="java.util.Date"/>
				...
				<display:column title="天數">
					<bean:define id="days" name='row' property='days' type="java.lang.Integer"/>
						<%=days %>
				</display:column>
					...
					<html:link action='/GAMA0904' name='paramMap'>
						<logic:notEmpty name='row' property='debtRate'>
							<bean:define id="debtRate" name='row' property='debtRate' type="java.math.BigDecimal"/>
								...
								<input type="hidden" name="commitFirstRate" value="<%=debtRate%>" />
						</logic:notEmpty>
						<logic:empty name='row' property='debtRate'>
							<p align="right"><%="0.000"%></p>
							<input type="hidden" name="commitFirstRate" value="0" />
						</logic:empty>	
					</html:link>	
				</display:column>	
				<display:column title="應付利息">
					<bean:define id="interestAmount" name='row' property='interestAmount' type="java.math.BigDecimal"/>
						<p align="right"><%= new java.text.DecimalFormat("#,##0").format(interestAmount)%></p>
				</display:column>
			</display:table>
		</td>
	</tr>
